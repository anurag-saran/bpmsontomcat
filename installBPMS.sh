...
usage() {
	echo "cd \$TOMCAT_HOME"
# Longer term TODOs
	echo "wget https://raw.githubusercontent.com/shuawest/bpmsontomcat/master/installBPMS.sh" 
	echo "chmod a+x installBPMS.sh"
	echo "./installBPMS.sh"
	echo " "	
...
if [ ! -f jboss-bpms-deployable-generic.zip ]; then
	...
	mv jboss-bpms-*-deployable-generic.zip jboss-bpms-deployable-generic.zip
fi
...
mv jboss-bpms-*-deployable-generic jboss-bpms-deployable-generic
pushd jboss-bpms-deployable-generic
unzip jboss-bpms-manager.zip
unzip jboss-bpms-engine.zip
popd
popd

cp -rv /tmp/jboss-bpms-*-deployable-generic/jboss-bpms-manager/business-central.war webapps/business-central
cp -rv /tmp/jboss-bpms-*-deployable-generic/jboss-bpms-manager/dashbuilder.war webapps/dashbuilder

cp -v /tmp/jboss-bpms-*-deployable-generic/jboss-bpms-engine/lib/btm*.jar ./lib/
cp -v /tmp/jboss-bpms-*-deployable-generic/jboss-bpms-engine/lib/jta*.jar ./lib/
cp -v /tmp/jboss-bpms-*-deployable-generic/jboss-bpms-engine/lib/slf4j-api*.jar ./lib/
cp -v /tmp/jboss-bpms-*-deployable-generic/jboss-bpms-engine/lib/slf4j-ext*.jar ./lib/
...
cp -v /tmp/ojdbc6.jar ./lib/
#cp -v /tmp/mysql-connector-java.jar ./lib/
